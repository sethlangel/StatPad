...
export default function StatsTable(props: { data: Stats[] }) {
    ...
    return (
        <div className="flex flex-col justify-center">
            <div className="overflow-x-auto rounded-lg shadow-lg">
                <table className="w-full divide-y divide-gray-300 dark:divide-gray-700 rounded-lg">
                    ...
                    <tbody className="bg-base-100 divide-y divide-gray-200 dark:divide-gray-700">
                        {props.data.length > 0 && props.data.map(({ error_name, total_error_count, error_rank }) => (
                            <tr key={error_name} className="hover:bg-gray-50 dark:hover:bg-gray-700">
            ...
            {props.data.length == 0 && <div className="text-4xl text-center mt-10">No data for selected date time range.</div>}
        </div>
    )
}
